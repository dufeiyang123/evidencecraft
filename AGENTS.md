...
以下产品方向已落实为当前 Skill 的职责和交接约束，不表示生产效果已经验证。不得仅凭静态
检查宣称数值稳定性、用户可读性或实际 token 节省；也不得据此扩大数据访问范围。

- 核心用户为中初级数据分析师，重点服务口径翻译、多表关联、报告撰写和定期维护。
  “覆盖日常 60% 以上工作”为待定义统计口径并验证的产品目标，不作为已实现的能力声明。
- 优先覆盖跨部门口径冲突与对账、零散临时分析、周期性重复分析及流程复用三类场景。
- 主要分析对象为数仓数据；输入也包括项目文档和优质历史分析报告，用于理解业务与选择
  适合当前问题的分析思路。核心 Skill 仍保持平台和连接器无关。
- 核心价值为口径锁定、流程管控、证据驱动和全链路可溯源，保留独立审查与证据链。
- 数值稳定性是底线：锁定指标口径、来源和计算实现。完整输入包括快照、参数、语义与代码
  版本及影响结果的执行设置；比较规范化业务数值。新建或修改实现由 Executor 验证，保留
  历史版本和更正关系；不通过重写口径掩盖代码错误。
- 分析视角、问题组织和报告表达允许按场景、用户需要及历史样例调整；明确区分数值计算的
  稳定要求与分析策略的灵活性。
- Plan 负责方案级调研：按需读取项目规则、已有实现和历史方法样例，完成范围与成本受限的
  通用健康检查。历史报告不自动授权旧口径或提供本期结论；只有真实语义缺口进入 profiling。
  关键检查受限时交付有限制的 Draft，说明影响和解除条件；不得将其作为可执行 Current Plan。
- Plan 决策完整不等于预先算出报告结论。单份 preparation 记录承接可复用观察，计算产出
  版本固定的实现和验证结果，写作据此组织解释；新数值回到计算环节，不增设写作 Skill。
- 暂不引入深度数据治理能力；本轮方向是通用规则校验，不扩展为数仓修复或治理平台。
- 延续可读性与成本目标，避免新增重复确认、重复检查和全量上下文加载。
...
planning 先读取相关项目资料并进行有限健康检查，再形成策略、报告蓝图和执行接口。检查方法
统一维护在 profiling 的 Reference，复用与计算/写作交接统一维护在 common Executor contract。
本轮保留九个 Skill，不新增健康报告、写作技能、固定审批点或可读性审查 Agent。宿主禁止写入时，
在会话中交付方案与精确引用；允许写入后保存，不虚构文件或提前建立 Run。

Report Plan 只推荐执行方式；新 Run 的路由沿用明确选择、适用的既有偏好，或授权范围内的可行默认，
仅在有实质成本/能力分歧或用户要求审批时询问，并在 progress 中冻结方式、依据、来源和时间。
Subagent-Driven 默认逐个派 fresh worker；只有用户对当前 Run 明确要求并行时，才由 `dispatching-parallel-research` 对真正独立的 ready Packages 做一次并发调度。
