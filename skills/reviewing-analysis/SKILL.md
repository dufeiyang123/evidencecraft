...
Before dispatch, verify that the manifest names the same current artifacts present on disk and that the Executor marked the run ready for independent review. Do not infer readiness from a draft alone.

| Observed state | Action |
|---|---|
| A Work Package output, evidence log, draft, or manifest is missing/incomplete | Return to the selected Executor. |
...
1. Copy [the reviewer prompt](prompts/analysis-reviewer.md) into an independent reviewer call.
2. Replace every placeholder with exact paths, identities, confirmed use, risk, Plan checks, save rule, and any prior review/correction records.
...
5. Require output conforming to [the Review Report template](assets/review-report-template.md).
...
Check required sections, clear distinction between observation and interpretation, decision-relevant caveats, and absence of unreviewed final/approved claims. Confirm the Plan's final destination and whether qualified reports may be saved.

## Classify findings by consequence

Use exactly these levels:

- `BLOCKING`: a wrong or unsupported consequential result; stale/mixed semantics; missing required evidence or section; hidden material limitation; unreconciled contradiction; or unsafe final-save condition.
...
| Defect | Return Route |
|---|---|
| Access, calculation, evidence capture, draft, or verification error under unchanged semantics | The Plan-selected Executor—`executing-report-plans` or `subagent-driven-reporting`—at the first affected Work Package. |
...
1. verify freshly that the draft, evidence log, semantic dependencies, and Review Package still match the identities the reviewer inspected;
2. preserve the Review Report at the Plan's risk-proportionate review path;
3. copy the exact reviewed draft bytes to the Plan's final destination without editorial changes;
4. verify the saved report identity matches the reviewed draft and record the verdict, Review Report path, final path, and identity in progress.

If any reviewed input changed, do not save; create a new Review Package and re-review. If saving would require adding a missing qualification or editing wording, return to the Plan-selected `executing-report-plans` or `subagent-driven-reporting` Executor and re-review the revised draft.
...
Return the verdict, Review Report path or complete compact record, all material findings with evidence, exact Return Routes, re-review requirements, and—only after a passing save gate—the final report path and verified identity.
